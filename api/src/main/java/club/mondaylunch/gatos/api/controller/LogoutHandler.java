...
public class LogoutHandler extends SecurityContextLogoutHandler {
    ...
    public void logout(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
        ...
        String issuer = (String) this.getClientRegistration().getProviderDetails().getConfigurationMetadata().get("issuer");
        String clientId = this.getClientRegistration().getClientId();
        ...
        this.log.info("Will attempt to redirect to logout URL: {}", logoutUrl);
        try {
            httpServletResponse.sendRedirect(logoutUrl);
        } catch (IOException ioe) {
            this.log.error("Error redirecting to logout URL", ioe);
        }
    }
