...
public class ValueProviderNodeTypeTest {
    ...
    public void isDefaultOutputValueCorrect() {
        BasicNodes.VALUE_PROVIDER_TYPES_WITH_DEFAULTS.forEach(databox -> {
            ...
            var node = Node.create(nodeType);
            Assertions.assertEquals(databox.value(), nodeType.compute(Map.of(), node.settings(), Map.of()).get("output").join().value());
        });
    }
    ...
    public void isOutputValueCorrect() {
        BasicNodes.VALUE_PROVIDER_TYPES_WITH_DEFAULTS.forEach(databox -> {
            ...
            Assertions.assertEquals(TEST_VALUES.get(databox.type()), nodeType.compute(Map.of(), node.settings(), Map.of()).get("output").join().value());
        });
    }
}
